 # AWS modules using Pulumi Template

 A minimal Pulumi template for provisioning different aws modules using Python.

 ## Overview

 This template provisions an different resources in your AWS account and exports its ID as an output. It’s an ideal starting point when:
  - You want to learn Pulumi with AWS in Python.
  - You need a barebones several aws resources deployment to build upon.
  - You prefer a minimal template without extra dependencies.

 ## Prerequisites

 - An AWS account with permissions to create different resources. Admin access is preferred .
 ...
 - Customize `__main__.py` to add or configure additional resources.
   - Upcoming ECS, EKS, Lambda, sqs, sns, route53 etc
